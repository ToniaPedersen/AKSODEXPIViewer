...
function parseVariantCondition(conditionObj, enumLiteralSymbols) {
    ...
    const valuePath = valueFromData(conditionObj, "Value");
    if (typeof propertyPath !== "string" || !propertyPath) return null;
    if (typeof valuePath !== "string" || !valuePath) return null;
    const attributeName = propertyPath.split(".").pop();
    const literalName = valuePath.split(".").pop();
    const value = enumLiteralSymbols.get(literalName) ?? literalName;
...
function parseLabelTemplateAlignment(alignmentRef) {
    ...
    const horizontal = /Left/.test(name) ? "Left" : /Right/.test(name) ? "Right" : "Center";
    const vertical = /Top/.test(name) ? "Top" : /Bottom/.test(name) ? "Bottom" : "Center";
...
function parseLabelTemplate(obj) {
    ...
    return {
        position: aggregatedValue(getData(obj, "Position")?.firstElementChild) || { x: 0, y: 0 },
        rotation: numberFromData(obj, "Rotation", 0),
        size: numberFromData(obj, "Size", 3.5),
        font: valueFromData(obj, "Font") || "Arial",
        color: aggregatedValue(getData(obj, "Color")?.firstElementChild) || { r: 0, g: 0, b: 0 },
        alignment: parseLabelTemplateAlignment(valueFromData(obj, "Alignment")),
        text,
        index: valueFromData(obj, "Index") || "",
    };
}
...
export function parseSymbolCatalogue(discDoc) {
    ...
    qsa(discDoc, 'Object[type="Profile/Symbol"]').forEach(obj => {
        ...
        const variants = directComponentsObjects(obj, "Variants").map((variant, i) => {
            const condObj = directComponentsObjects(variant, "Condition")[0] || null;
            return {
...
export function collectGraphicalElements(mainDoc, symbolMap, discDoc = null) {
    ...
    function pickVariant(symbol, representedId) {
        ...
        // No condition matched - fall back to the base (VariantNumber 0) variant,
        // then any unconditioned variant, then the first as a last resort.
